...
appid = "5718460805"    # 项目的 appid
token = "vFzcvcXIxKsvAjcqH5LgYeLLtf_gMfPA"    # 项目的 token
cluster = "volcengine_streaming_common"  # 请求的集群
audio_path = "D:\\Docs\\Tencent Files\\2963959417\\nt_qq\\nt_data\\Ptt\\2025-05\\Ori\\aa68a90667ca8970fbdadd02b3ec0ecd.mp3"  # 本地音频路径
    ...
    async def segment_data_processor(self, wav_data: bytes, segment_size: int):
        ...
        async with websockets.connect(self.ws_url, additional_headers=header, max_size=1000000000) as ws:
            ...
            for seq, (chunk, last) in enumerate(AsrWsClient.slice_data(wav_data, segment_size), 1):
                ...
                #text = audio_only_request.decode('utf-8')
                #print(text)
...
async def deal_audio_recognize(filepath):
    """
    处理音频识别
...
def silk_v3_to_mp3(input_file, delete_after_finish=False):
    """
    将Silk V3音频文件转换为MP3格式
    
    参数:
        input_file: 输入Silk文件路径
        delete_after_finish: 转换完成后是否删除原始文件
    
    返回:
        转换后的MP3文件路径，失败时返回空字符串
    """
    ...
    pcm_file = os.path.join(dir_name, f"{file_name}.pcm")
    output_file = os.path.join(dir_name, f"{file_name}.mp3")
    
    # Silk解码器路径和FFmpeg路径（根据实际情况修改）
    silk_decoder = r"D:\Downloads\_software\silk-v3-decoder-master\windows\silk_v3_decoder.exe"
    ffmpeg = r"D:\ffmpeg\bin\ffmpeg.exe"
    
    try:
        time.sleep(0.8)
        # 第一步：使用silk解码器将Silk转换为PCM
        silk_cmd = [silk_decoder, input_file, pcm_file]
        subprocess.run(silk_cmd, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        
        # 第二步：使用FFmpeg将PCM转换为MP3
        ffmpeg_cmd = [
            ffmpeg,
            "-f", "s16le",  # PCM格式：16位有符号小端
            "-ar", "24000", # 采样率24kHz
            "-ac", "1",     # 单声道
            "-i", pcm_file,
            "-codec:a", "libmp3lame",  # MP3编码器
            "-q:a", "2",               # 质量等级2（约192-256kbps）
            "-y",                      # 覆盖已存在文件
            output_file
        ]
        ...
        # 如果需要，删除原始Silk文件
        if delete_after_finish and os.path.exists(input_file):
            os.remove(input_file)
            
        return output_file
        
    except subprocess.CalledProcessError as e:
        logger.warning(f"转换失败: {e.stderr.decode('utf-8')}")
        return ""
